...
For plans with red→green test cycles, use `/tdd-issue` instead.
...
Read the plan in full before doing anything else. If the plan has a "TDD Order" section with red→green test cycles, stop and tell the user to run `/tdd-issue` instead.
...
2. **Verify.** Run the linters to confirm the change is clean:
   - `pnpm run lint:all` (Biome + markdownlint + import check).
...
2. If any `.ts` files were touched, run the type check: `pnpm run build` (`tsc -p tsconfig.json`, `noEmit: true`). Must succeed.
