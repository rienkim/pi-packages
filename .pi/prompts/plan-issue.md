...
Your job is to produce a numbered implementation plan at `docs/plans/NNNN-<slug>.md` for issue #$1, then commit it. Stop after the commit. Do **not** start implementation — the next step is `/tdd-issue` (for plans with test cycles) or `/build-issue` (for docs-only or non-code changes).
...
5. Open the source files most relevant to the change and skim them before writing. Common entry points: `src/extension.ts`, `src/tools/` (Pi wrappers), `src/lib/ci.ts`, `src/lib/release.ts`, `src/lib/issue.ts`, `src/lib/github.ts`, `src/lib/ci-helpers.ts`, `src/lib/config.ts`, `src/lib/process.ts`, `src/progress.ts`.
...
- If the plan adds fields to shared interfaces or touches tool/lib wiring: load the `design-review` skill and run its checklist on the affected modules.

## Decide

Before writing the plan, identify any genuinely ambiguous design choices. If there are 1–2 such choices (API shape changes, breaking vs non-breaking, tool naming, structured output format, etc.), use the `ask-user` skill once to surface them with a short context summary and concrete options. Skip this step if the issue's "Proposed change" section is unambiguous.
...
- **Background** — relevant existing modules/functions and how they relate. Note whether the change touches tool wrappers (`src/tools/`), portable business logic (`src/lib/`), or the extension entry point (`src/extension.ts`). Flag any constraint from AGENTS.md that applies (e.g., no Pi SDK imports in `src/lib/`).
- **Design Overview** — decision model, data shapes, separation of concerns, edge cases. Include code-fenced TS types when shape changes. Note any impact on the `onProgress`/`onUpdate` mapping in `src/progress.ts`.
- **Module-Level Changes** — file-by-file list of what's added, changed, or removed across `src/`, `tests/`, and `docs/` (flag any architecture doc that describes a module, type, or flow being changed).
...
If the change is breaking (tool name change, structured output shape change, config key rename, etc.), say so explicitly in Goals and use `feat!:` in the suggested commit messages.
